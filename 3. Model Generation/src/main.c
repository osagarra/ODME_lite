/********************************************************************************
 * 
 *	GENERATE Expectations for various Network models
 *
...
int main(int argc, char *argv[]){
	...
	printf(\
	"################################\n"
	"########## Multi-Edge Network Model Generator ###########\n"
	"####################################\n");
 /***********************************************************************
 ...
 ************************************************************************/
    ...
    int opt_indist  	= 0; // indist. option (only for fixed k)
	...
	int ch;
	        ...
	        while ((ch = getopt(argc, argv, "N:T:g:C:f:a:Q:d:z:L:c:l:S:p:v:s:e:x:h:m:r:R:")) != -1) {
	             switch (ch) {
			     case 'N': /* N_nodes */
			             N_nodes=atoi(optarg);
						 break;
	...
	if((N_nodes<0)||(opt_dir<0) || (cases<0))
	{
 		fprintf(stderr,	"\nCorrect usage is: ./simus -args \n\nWhere:\n\n"
				...
				//" *		-D Indistiguishable option (only for case of fixed degrees, see README) (>0 for indistinguishable weights) (Default =0) \n"
	...
	if(seed==1)
	{
		...
		if(opt_verbose>1) printf("Default seed given, using processor time...\n");
	}
		...
		else if(cases==1) // fixed pij case
		{
			...
			if(cases==2)
			{
				...
				if(opt_dir==1)
				{
					...
					ymax = max_value_double(x2[1],N_nodes); // x  = s / sqrt(T)
                    T = sum_vec_double(x2[0],N_nodes);
                    T = T*sum_vec_double(x2[1],N_nodes);
					w_max=10* xmax*ymax;
				}else{
				...
				if(opt_dir==1)
				{
					...
					w_max=10*xmax*ymax*exp(xmax*ymax)*gamma/(1+gamma*(exp(xmax*ymax)-1));
				}else{
					x2 = read_node_list_xatts_double(file_s, N_nodes, 1, header);
					xmax = max_value_double(x2[0],N_nodes);
					w_max=10*xmax*xmax*exp(xmax*xmax)*gamma/(1+gamma*(exp(xmax*xmax)-1));
				}
                if(gamma<0)
                {
				...
				if(opt_dir==1)
				{
					...
					w_max=(int)(10*xmax*ymax*exp(xmax*ymax)*zmax*wmax/(1+zmax*wmax*(exp(xmax*ymax)-1)));
				}else{
					x2 = read_node_list_xatts_double(file_s, N_nodes, 2, header);
					xmax = max_value_double(x2[0],N_nodes);
					zmax = max_value_double(x2[1],N_nodes);
					w_max=(int)(10*xmax*xmax*exp(xmax*xmax)*zmax*zmax/(1+zmax*zmax*(exp(xmax*xmax)-1)));
				}
			}else{
                meth = 2;
				...
				rho = w_graph_compute_rho(E_av,(int)T,opt_indist);
				printf("\t\t Total expected edges: %f, Total sampling:%f, average edge weight: %f\n",E_av,T,T/E_av);
				if(opt_dir>0)
				{
					w_max = 10 * T/E_av * (xmax*ymax/(1.+xmax*ymax));
				}else{
					w_max = 10 * T/E_av * (xmax*xmax/(1.+xmax*xmax));					
				}
			}
		}
	}
 ...
 ************************************************************************/
	...
	for(r=0;r<reps;r++)
	{
		if(cases==0) // other models
		{
			...
			entropy_seq[r]= w_graph_entropy_multi_edge(WG,N_nodes); // assuming poisson case
			...
			if(opt_dir==1)
			{
				WG = fixedEs_poisson_directed_graph(x2, gamma, N_nodes, randgsl, opt_verbose, opt_self, max_reps);
			}else{
				WG = fixedEs_poisson_undirected_graph(x2[0],  gamma, N_nodes, randgsl, opt_verbose, opt_self, max_reps);
			}
            entropy_seq[r] = w_graph_entropy_ZIP(WG, N_nodes, x2, gamma, opt_self, opt_dir);
            ...
            entropy_seq[r] = w_graph_entropy_ZIP2(WG, N_nodes, x2, opt_self, opt_dir);
		}else{ // fixed k
			if(opt_verbose==1) printf("============### ZIP Poisson model ####===========\n"); fflush(stdout);
			if(opt_dir==1)
			{
            ...
            entropy_seq[r] = w_graph_entropy_binary(WG, N_nodes, x2, opt_self, opt_dir);
		}
