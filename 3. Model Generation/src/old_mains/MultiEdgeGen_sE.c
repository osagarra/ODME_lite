 ...
 *
 *	This program generates networks (and statistics) with pre-defined node strenghts and total number of Edges using a
 *  macro-canonical approach.
 *     This implementation is about the case where one fixes the total number of edges and strength sequence
 *  More details in
...
int main(int argc, char *argv[]){
	...
	printf(\
	"################################\n"
	"########## Maximum entropy multi-edge networks: Fixed total edges, fixed strengths ###########\n"
	"####################################\n");
 /***********************************************************************
 ...
 ************************************************************************/
	...
	int ch;
	        while ((ch = getopt(argc, argv, "N:s:d:f:x:v:c:l:h:T:g:r:p:e:")) != -1) {
	             switch (ch) {
	             case 's': /* seed */
	                     seed=atoi(optarg);
	                     break;
	...
	if((N_nodes<0)||(opt_dir<0)|| (T<0))
	{
 		fprintf(stderr,	"\nCorrect usage is: ./simus -args \n\nWhere:\n\n"
				...
				" *		-g Lambda value (E lagrange multiplier, default=0.0)\n"
...
//				" *		-m Maximum distance for binning (default= 20000) [in meters]\n"
	...
	if(lambda<=0)
	{
		printf("Lambda must be >0, specify it! Aborting...\n");
		abort();		
	}
 ...
 ************************************************************************/ 	
	...
	if(opt_dir==1)
	{
		...
		w_max=(int)(50*xmax*ymax*exp(xmax*ymax)*lambda/(1+lambda*(exp(xmax*ymax)-1)));
	}else{
		xmax = max_value_double(x,N_nodes);
		w_max=(int)(50*xmax*xmax*exp(xmax*xmax)*lambda/(1+lambda*(exp(xmax*xmax)-1)));
		if(opt_clust==1)
		{
			len_acc_nodes=9;
		}else{
 ...
 ************************************************************************/
	...
	for(r=0;r<reps;r++)
	{
		...
		if(opt_dir==1)
		{
			WG = fixedEs_poisson_directed_graph(x2, lambda, N_nodes, randgsl, verbose, self_opt, max_reps);
		}else{
			WG = fixedEs_poisson_undirected_graph(x,  lambda, N_nodes, randgsl, verbose, self_opt, max_reps);
		}
